...
def main():
    ...
    lebanon_data = data[data['country'] == campus].iloc[:, 1:]
    ...
    years = lebanon_data.columns.to_numpy(dtype=int)

    lebanon_pop = [preprocess_population(pop) for pop in lebanon_pop]
    france_pop = [preprocess_population(pop) for pop in france_pop]
    hungary_pop = [preprocess_population(pop) for pop in hungary_pop]

    plt.plot(years, lebanon_pop, label=campus)
    ...
    plt.legend()
    plt.tight_layout()
    max_pop = max(max(lebanon_pop), max(france_pop), max(hungary_pop))
